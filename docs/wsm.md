...
```
bin/wsm-core          # ロジック層 (JSON API)。ホストのみに配置
...
| SessionManager | Workspace につき 1 セッション |
...
セッション名は `(ns_repo, id)` からドメイン層が導出する。実装を切り替えても
既存セッションを見失わないよう、導出規則は実装側に持たせない。
