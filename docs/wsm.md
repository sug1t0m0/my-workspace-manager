...
トラッカーは**インスタンス**として複数登録できる (同じバイナリを別名・
別設定で。例: 個人の GitHub と organization の GitHub)。

- repo-group の一覧 (list-repo-groups) は**全インスタンスに並列で照会して
  束ね**、各グループがインスタンス名を名乗る。個々のインスタンスの失敗は
  そのグループが欠けるだけ。トラッカーが 1 つも未設定のときは縮退させず
  エラーにする (対話フローの入り口で設定誤りを表面化させる)
- グループ単位の照会 (list-group-issues / list-repos の group フィルタ) は
  `--tracker` で指定されたインスタンス (省略時は既定)
- リポジトリ単位の照会 (list-issues 等) の解決は
  **`[[repo]].tracker` (明示) > ns マッピング > 既定** の優先順位。
  トラッカーが全く未設定なら空に縮退する (Tracker なしでも main の開閉は
  できるべき)
...
- `wsm-tracker-github-api` (crates/tracker-github-api、**推奨**): GraphQL API
  ...
  sub-issues (Issue の親子関係) とページングに対応。
  接続先は `WSM_TRACKER_GITHUB_API_URL` で差し替え可能 (契約テスト用)。
  インスタンス設定 ([[tracker]] の拡張キー):
  - `owner`: repo-group (Projects) の照会先 (個人 / organization。省略時は
    認証ユーザー)
  - `root_label`: グループの Issue 一覧を、このラベルの付いたルート Issue
    (エピック等) だけに絞る。組織の Project は全員の項目が並んでノイジーな
    ため、GitHub 側でラベルを付けて境界を運用し、open な子孫は既存の
    階層ドリルで辿る。グループの Issue 照会は Project items の走査ではなく
    検索 API (`is:issue is:open project:<owner>/<id> [label:"…"]`) を使う —
    ほぼ closed の大きなボードでも 1 クエリで返る
  - `groups`: 表示する repo-group (Project) の id をカンマ区切りで限定する。
    組織の全 Project を出すと多すぎる場合に、関係のあるボードだけを出す
...
`session list --json` + running セッションごとの `workspace list`)と照合し、
外部コマンドの起動回数を Issue 数に依らず固定にする(全行が同じ時点の状態で
判定される)。スナップショットは取り込み後に変化しない(遅延取得・再取得を
しない)。寿命は 1 コマンドの処理中だけで、プロセス外には持ち出さない。
状態を変える経路 (open / remove) は変更直後の再確認が要るため、その都度
live に照会する。
...
| `[[tracker]]` | Tracker プラグインのインスタンスの列挙 (複数可。同じバイナリを別名・別設定で複数登録できる)。予約キーは `name` (必須)、`path` (実行ファイル、必須)、`ns` (カンマ区切り。この namespace のリポジトリの既定トラッカーになる。割り当ての重複はエラー)。**それ以外のキーは環境変数 `WSM_TRACKER_<KEY大文字>` としてプラグインに渡る** (例 `owner = "acme"` → `WSM_TRACKER_OWNER=acme`。wsm は値の意味を知らない) |
...
`list-repo-groups`
→ open な repo-group の一覧 (**全トラッカーに並列照会して集約**。並びは
既定トラッカーが先頭、あとは設定順)。id は不透明な文字列で、tracker が
どのインスタンスのグループかを示す。トラッカー未設定はエラー
(`no tracker configured ...`)。個々のインスタンスの失敗はそのグループが
欠けるだけ。
```json
[{"id": "1", "title": "...", "tracker": "github-me"}]
```
...
トップレベル。main はリポジトリ単位の概念なので出ない。**親が同じ応答に
居る item** (GitHub は sub-issue を親と同じ Project に自動追加する) は
ドリルとの重複を避けるためトップに出さず、作業中のものだけ `orphan: true`
で浮上する。親が応答に居ない item はサブツリーの入り口として残る
(親を持つというだけで一律に隠すと、ルートが Project の item でない構成で
サブツリーごと不可視になるため)。プラグイン非対応時は空
(UI はリポジトリ起点フローに落ちる)。
```json
...
常に false)。`orphan` は孤児 worktree の行 (一覧の外から浮上した作業中の
workspace) を示し、UI は階層の一部と誤読されないよう行頭 `↳` で描き分ける。`repo` は各 Issue の所属リポジトリ (クロスリポジトリの
...
- 引数はパラメータごとに形まで検証する(SSH 経由で呼ばれるため必須。
  ...
  - `tracker`: 英数と `-`(先頭は英数)。トラッカーのインスタンス名
...
- 認証情報・接続先などトラッカー固有の設定はプラグインが自分で読む。
  wsm は値の意味に関知しない
- 起動時の環境変数: `WSM_TRACKER_NAME` (インスタンス名) と、`[[tracker]]`
  の予約外キー `WSM_TRACKER_<KEY大文字>` が渡る。同じバイナリが複数
  インスタンス (個人用 / organization 用など) として登録され得るため、
  インスタンスごとの設定はこの経路か、`WSM_TRACKER_NAME` を鍵にした
  自前の設定ファイルで受け取る
...
| `list-group-issues-v0 --group <id> [--cursor <token>]` | `{"issues": [{"id", "title", "repo", "has_children", "parent"}], "next_cursor"}` — repo-group に属する open な Issue の 1 ページ (リポジトリ横断。`repo` 必須。`parent` (任意) は親 Issue の識別 `{"repo", "id"}`) |
