...
crates/tracker-github   # 公式 GitHub Tracker プラグイン (gh CLI)。契約だけで結合
...
| ロール | 責務 | 層 | 現在の実装 | 追加の例 |
|---|---|---|---|---|
| Tracker | Project / リポジトリ / Issue の照会 | server | プラグイン (公式: wsm-tracker-github) | Jira, Linear, GitLab Issues (自作プラグイン) |
...
- プロジェクト照会 (list-projects / list-repos の Project フィルタ) は
...
特定トラッカーの仕様に寄せない。Project 概念を持たないトラッカーは
list-projects が空を返してよい(UI には「Project: none」で全リポジトリを
出す経路が既にある)。

公式プラグイン: `wsm-tracker-github` (crates/tracker-github。gh CLI を使う)。
Project のリポジトリ解決は GraphQL の `repositoryOwner` + inline fragment を
使い、`ns` が個人 (User) でも organization でも同じように動く。owner は
gh の認証ユーザーを自己解決し、`WSM_TRACKER_GITHUB_OWNER` で上書きできる。
...
`list-projects`
→ open なプロジェクトの一覧 (既定トラッカー)。id は不透明な文字列。
トラッカー未設定はエラー (`no tracker configured ...`)。
```json
[{"id": "1", "title": "..."}]
```

`list-repos --project <id|none>`
→ リポジトリ一覧。`none` は RepoStore (ghq + 設定 `[[repo]]`) の
全リポジトリ、id 指定時はそのプロジェクトに属し RepoStore にもあるもの
...
並びは worktree 一覧順)。RepoStore で解決できないリポジトリはエラー
...
- 引数はパラメータごとに形まで検証する(SSH 経由で呼ばれるため必須。
  ...
  - `project`: issue と同じ不透明な id の文法(`none` は list-repos の特別値)
...
|---|---|
| `list-projects-v0` | `[{"id": "...", "title": "..."}]` — open なプロジェクト。UI の表示順で返す |
| `project-repos-v0 --project <id>` | `["ns/repo", ...]` — プロジェクト所属のリポジトリ |
| `list-issues-v0 --repo <ns/repo>` | `[{"id": "...", "title": "..."}]` — open な Issue。UI の表示順で返す |
...
- プロジェクト概念を持たないトラッカーは `list-projects-v0` で `[]` を返す
...
- `id` は Issue id の文法 (英数と `-`、先頭英数) で検証し、違反する要素と
  ...
  流れ込むため、ここが注入対策の関所
- `ns/repo` は RepoRef の文法で検証する (`project-repos-v0` の応答)
...
- 起動方法など動詞で表現できない変更が必要になったら、`info` 動詞の追加
  (互換な変更) で握手プロトコルへ移行できる
