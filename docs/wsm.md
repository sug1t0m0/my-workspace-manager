...
crates/core           # ロジック層の Rust 版 (試運転中)。zsh 版と JSON API 互換
crates/shared         # core と (将来の) client が共有する語彙 (domains)
...
crates/core/src/presentations/  # CLI: 引数 → ドメイン型、結果 → JSON / exit code
crates/core/src/usecases/       # オーケストレーション (依存の順序と合成ビュー)
crates/core/src/roles/          # Tracker / RepoStore / SessionManager / Worktree / DevContainer
crates/core/src/infra/          # exec (プロセス起動), settings (config.toml)
...
Rust 版は `crates/core` に置き、完成まで zsh 版 (`bin/wsm-core`) と
...
JSON API の契約テストを `crates/core/tests/contract/` に置く。
