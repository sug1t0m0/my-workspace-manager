...
- ensure: セッションが running でなければ `herdr --session <name> server` を
  ...
  workspace を `--cwd <workspace パス> --label <ラベル> --focus` で作成し、
  既存なら `workspace focus` する (open = そこに向かう、の意図に合わせる)。
  ラベルは main = リポジトリ名、Issue = Issue 番号。**main も workspace を
  保証する**: ヘッドレス起動直後のセッションは workspace ゼロで、アタッチ時の
  自動作成に任せると cwd がリポジトリにならない (ホームが開く) ため
