...
```
bin/wsm-server          # サーバー (JSON API)。zsh 版 (現行)。ホストのみに配置
bin/wsm               # クライアント。ホスト・DevContainer 共通の単一実装
crates/server           # サーバーの Rust 版 (試運転中)。zsh 版と JSON API 互換
...
| ロール | 責務 | 層 | 現在の実装 | 追加の例 |
|---|---|---|---|---|
| Tracker | Project / リポジトリ / Issue の照会 | server | GitHub (gh) | Jira, Linear, GitLab Issues |
| RepoStore | ローカルクローンの列挙とパス解決 | server | ghq | 任意の配置規約 |
...
→ open な GitHub Project の一覧。`--user` 省略時は server が gh で自己解決する。
```json
[{"number": 1, "title": "..."}]
```
...
クライアントは変更なしで差し替えられること。UI まで取り込むかは server 置き換え後に
判断する。

Rust 版は `crates/server` に置き、完成まで zsh 版 (`bin/wsm-server`) と
...
UI に Rust 版を掴ませる(UI は PATH 上の `wsm-server` を呼ぶだけなので無変更)。
...
- テスト対象は既定でビルドした Rust 版。
  `WSM_SERVER_BIN=$PWD/bin/wsm-server cargo test --test contract` で
...
### gh の呼び出しは server に閉じる

クライアントは Tracker (現在は gh) を直接呼ばない。GitHub ユーザーの解決は server が
行う(`--user` 省略時に server が自己解決)。これにより DevContainer からの
SSH ホワイトリストは `wsm-server` エントリだけで足りる。
