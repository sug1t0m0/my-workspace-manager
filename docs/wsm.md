...
| `[[repo]]` | ghq 管理外のリポジトリの登録 (複数可)。キーは `path` (クローンの場所、必須)、`host` (worktree 導出用、必須)、`ns` (必須)、`name` (省略時 path の basename)。必須キーの欠落や形の不正は error JSON (黙って捨てない) |
...
全リポジトリ、番号指定時はその Project に属し RepoStore にもあるもの
(`--user` は番号指定時のみ使用、省略時は自己解決)。`active_count` は
アクティブな Workspace 数。
```json
